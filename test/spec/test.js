...
(function() {
	describe('Sprites Test', function() {
		...
		beforeEach(function() {
			...
			Q = Quintus().include("Sprites, Scenes, Input, 2D, Touch, UI, Audio, Anim").include("SpaceInvadersScenes, SpaceInvadersModels, GunControls, SpaceInvadersAssets").setup(canvas).controls();
			Q.clearColor = "#CCC";
			Q.clear();
		...
		describe('Cannon', function() {
			it('should have the right asset', function() {
				runs(function() { // wait for Quintus to have finished
					// initialization.
			...
			it('alien should be able to shoot', function() {
				runs(function() {
			...
			it('one shoot should be alive', function() {
				runs(function() {
					...
					var shotCounter = Q('CannonShot').items.length + Q('AlienShot').items.length;

					expect(shotCounter).toBe(1);
				});
			});

			it('game over screen should be visible', function() {
				runs(function() {
